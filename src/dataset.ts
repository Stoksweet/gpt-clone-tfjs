...
export async function CharDataset(args: DatasetParams): Promise<Dataset> {
    ...
    const stoi = Object.fromEntries(chars.map((ch, i) => [ch, i + indexShift])); // For encoding char to tokens
    const itos = Object.fromEntries(chars.map((ch, i) => [i + indexShift, ch])); // For decoding tokens to chars
