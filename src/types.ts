...
export type ModelParams = {
    ...
    embdDropout?: number; // Embeddings dropout
    residDropout?: number; // Dropout applied to the residual (skip) connections. residDropout is applied to the output of a sub-layer (attention or FFN) before it is added to the residual stream.
    attnDropout?: number; // Attention Dropout
};
...
export type DatasetGetBatchParams = {
    split: 'train' | 'test';
    blockSize: number;
    // How many independant sequences are processed in parallel
    batchSize: number;
};
...
export type TrainingParams = {
    ...
    blockSize: number;
    // How many independent text sequences will be processed in parellel
    batchSize: number;
};
